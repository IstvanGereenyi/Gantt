...
# --- JELENLEGI IDŐ AUTOMATIKUS BEÁLLÍTÁSA (2026) ---
...
if 'settings' not in strx.session_state:
    strx.session_state.settings = {
        "Project_Title": "AKTUÁLIS PROJEKT - 2026",
        "MKT_Color": "#E64646", "MKT_Label": "Marketing",
        "FIN_Color": "#E69646", "FIN_Label": "Finance",
        "ENG_Color": "#34D05C", "ENG_Label": "Engineering",
        "PROD_Color": "#34D0C3", "PROD_Label": "Production",
        "IT_Color": "#3475D0", "IT_Label": "IT"
    }

# --- ELRENDEZÉS (Két oszlop: Balra az adatok, Jobbra a grafikon) ---
...
with col_left:
    ...
    # Projekt cím
    strx.session_state.settings["Project_Title"] = strx.text_input("Projekt címe:", strx.session_state.settings["Project_Title"])
    
    # Interaktív Excel-szerű táblázat a feladatoknak
    strx.subheader("Feladatok listája")
    strx.caption("Kattints duplán bármelyik cellára a módosításhoz! Új sort a táblázat alján lévő '+' gombbal adhatsz hozzá.")
    ...
    strx.session_state.tasks_df = edited_df

    # Színek beállítása színválasztóval
    strx.subheader("Részlegek színei és feliratai")
    depts = ["MKT", "FIN", "ENG", "PROD", "IT"]
    c1, c2 = strx.columns(2)
    for i, dept in enumerate(depts):
        target_col = c1 if i % 2 == 0 else c2
        with target_col:
            strx.session_state.settings[f"{dept}_Label"] = strx.text_input(f"{dept} név:", strx.session_state.settings[f"{dept}_Label"])
            strx.session_state.settings[f"{dept}_Color"] = strx.color_picker(f"{dept} szín:", strx.session_state.settings[f"{dept}_Color"])

with col_right:
    strx.header("📈 Gantt Diagram")
    
    try:
        df = strx.session_state.tasks_df.copy()
        df['Start'] = pd.to_datetime(df['Start'])
        df['End'] = pd.to_datetime(df['End'])
        df['Completion'] = pd.to_numeric(df['Completion'])
        
        color_dict = {dept: strx.session_state.settings[f"{dept}_Color"] for dept in depts}
        label_dict = {dept: strx.session_state.settings[f"{dept}_Label"] for dept in depts}
        project_title = strx.session_state.settings["Project_Title"]
        ...
        df['color'] = df.apply(lambda r: color_dict.get(str(r['Department']), '#808080'), axis=1)

        current_num = (MAI_DATUM - proj_start).days

        # Matplotlib kirajzolás (Világos háttér)
        ...
        # Alsó tengely
        max_days = max(df.end_num.max(), current_num + 3)
        ...
        # Jelmagyarázat
        legend_elements = [Patch(facecolor=color, label=label_dict[dept]) for dept, color in color_dict.items() if dept in df['Department'].unique()]
        ...
        # Diagram beküldése a weboldalra
        strx.pyplot(fig)
        plt.close(fig)
        
    except Exception as e:
        strx.error(f"Hiba történt a grafikon generálása közben. Ellenőrizd a dátum formátumokat (YYYY-MM-DD)! Hiba üzenet: {e}")