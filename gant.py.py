...
# --- INITIALIZATION (Csak a legelső futáskor fut le) ---
...
# --- CALLBACKS A MENTÉSHEZ ---
def on_cell_edit():
    # Amikor a felhasználó belenyúl a táblázatba, ez a függvény azonnal menti a session_state-be
    if strx.session_state.editor_key["edited_rows"] or strx.session_state.editor_key["added_rows"] or strx.session_state.editor_key["deleted_rows"]:
        # Frissítjük a session_state-ben tárolt DataFrame-et az editor aktuális állapotával
        strx.session_state.tasks_df = strx.data_editor(strx.session_state.tasks_df, num_rows="dynamic", key="temp_key")
        # Kis trükk az azonnali újratöltés kényszerítésére
        strx.rerun()
...
with col_left:
    ...
    # Az adatszerkesztő, ami a változásokat azonnal rögzíti a session_state-be
    ...
    # Folyamatosan szinkronban tartjuk a háttértárat az új adatokkal
    strx.session_state.tasks_df = edited_df

    # Színek és feliratok mentése
...
with col_right:
    ...
    try:
        ...
        date_range = pd.date_range(proj_start, periods=max_days + 15) if not df.empty else pd.date_range(MAI_DATUM, periods=15)
